...
function forEachFiberSetCost(fiber){
  ...
  for(var i in fiber.geometry.coordinates){
    console.log(fiber.geometry.coordinates[i])
    var newDist = geolib.getDistance(store.selectedPosition, fiber.geometry.coordinates[i])
    if(newDist<dist){
      dist = newDist
    }
  }
